...
def get_dataset_from_reader(savefile, userid, datasetype, lang):
    ...
    sysql_cmd = "SELECT contents_metacontent.id, contents_content.levelstat_ponddy , contents_metacontent.permanent_link, contents_metacontent.title,\
                 ...
                 contents_metacontent.simp_trad, contents_metacontent.owner_id, contents_content.content_segment FROM %s LEFT JOIN \
    ...
    linelists = ['Label,ID,Length,0,1,2,3,4,5,6,None']
    ...
    for row in rows:
        ...
        if owener_id in allow_owner_ids:
            ...
            ponddy_level_distributed = row[1]
            ...
            levelsts = []
            for xr in ['0', '1', '2', '3', '4', '5', '6', 'None']:
                levelsts.append('%s' % ponddy_level_distributed.get(xr))
            levelstrs = ','.join(levelsts)
            if datasetype == 'STB':
                # 標記從STB過來的資料
                if READER2STB.get(readerid, ''):
                    addline = 'Lv.%s,Pondlet_%04d_%06d,%s,%s' % (ponddy_level, READER2STB[readerid], readerid, len(''.join(seglists)), levelstrs)
                # 單純於Reader新增的資料
                else:
                    continue
            ...
            # 額外幫國防部(DOD)新增的資料
                ...
                addline = 'Lv.%s,dod-gloss_%06d,%s,%s' % (ponddy_level, readerid, len(''.join(seglists)), levelstrs)
