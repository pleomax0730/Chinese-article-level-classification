...
dataset = load_dataset(
    "csv",
    data_files={
        "train": "datasets/pondlet_STB_HSK3_20220714_content_data_train.csv",
...
def compute_metrics(eval_pred: EvalPrediction):
    ...
    report = classification_report(
        eval_pred.label_ids,
        preds,
        target_names=["Lv.1", "Lv.2", "Lv.3", "Lv.4", "Lv.5", "Lv.6"],
    )
...
for array in cf_arrays:
    df_cm = (
        pd.DataFrame(
            array,
            index=["Lv.1", "Lv.2", "Lv.3", "Lv.4", "Lv.5", "Lv.6"],
            columns=["Lv.1", "Lv.2", "Lv.3", "Lv.4", "Lv.5", "Lv.6"],
        ),
    )
